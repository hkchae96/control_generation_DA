...
def top_k_filtering(logits, top_k, probs=False):
    """
    masks everything but the topt k entries as -infinity
    used to mask logits such that e^-inf-->0 won't contribute to the sum of denominator
    """
    if top_k==0:
        return logits
    else:
        ...
        if probs:
            return torch.where(logits < batch_mins, 
                        torch.ones_like(logits)*0, logits)
        return torch.where(logits < batch_mins, 
                        torch.ones_like(logits) * -1e10, logits)

def top_k_top_p_filtering(
    logits, top_k, top_p, filter_value=-float("Inf"), min_tokens_to_keep= 1):
...
class PreTrainedModel(nn.Module, ModuleUtilsMixin):
    ...
    @classmethod
    def from_pretrained(cls, pretrained_model_name_or_path, load_in_half_prec=False, force_download=True, *model_args, **kwargs):
    ...
    def generate(
        ...
        if input_ids is not None:
            ...
            # input_ids shape : ([1,sequence length]), batch_size=1
        else:
            batch_size = 1
    ...
    def _generate_no_beam_search(
        ...
        while cur_len < max_length:
            ...
            if do_sample:
                ...
                next_token_logits = top_k_top_p_filtering(next_token_logits, top_k=10, top_p=top_p)
                # next_token_logits = top_k_filtering(next_token_logits, top_k=10)
                # Samples
