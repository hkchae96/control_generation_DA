...
# from calculate_diversity import preprocess_text_to_examples
...
'''
def	sentence_generation(args, model, tokenizer):
    ...
    print(args.prompt, args.control_code)
...
def train_discriminator(discriminator, optimizer, source_dataset, target_dataset, d_steps, epochs):
    ...
    source_dataloader = DataLoader(source_dataset, sampler=source_sampler, batch_size=100)

    target_sampler = RandomSampler(target_dataset)
    target_dataloader = DataLoader(target_dataset, sampler=target_sampler, batch_size=100)
...
#train_dataset : ./data/AG-news/cached_train_gpt2-medium_192_sst-2
def train(args, train_dataset, source_dataset, model, discriminator, tokenizer):
    """ Train the model """
    if args.local_rank in [-1, 0]:
        tb_writer = SummaryWriter()

    args.train_batch_size = args.per_gpu_train_batch_size * max(1, args.n_gpu) #2*n_gpu
    ...
    optimizer_gedi = AdamW(optimizer_grouped_parameters, lr=args.learning_rate, eps=args.adam_epsilon)
    scheduler = get_linear_schedule_with_warmup(
        optimizer_gedi, num_warmup_steps=args.warmup_steps, num_training_steps=t_total
    )
    ...
    if os.path.isfile(os.path.join(args.model_name_or_path, "optimizer.pt")) and os.path.isfile(
        ...
        optimizer_gedi.load_state_dict(torch.load(os.path.join(args.model_name_or_path, "optimizer.pt")))
    ...
    if args.fp16:
        ...
        model, optimizer_gedi = amp.initialize(model, optimizer_gedi, opt_level=args.fp16_opt_level)
    ...
    logger.info("  Num examples = %d", len(train_dataset)) # 240000
    logger.info("  Num Epochs = %d", args.num_train_epochs) # 1
    logger.info("  Instantaneous batch size per GPU = %d", args.per_gpu_train_batch_size) # 2
    logger.info(
        "  Total train batch size (w. parallel, distributed & accumulation) = %d", # 6
    ...
    pt_id = tokenizer.encode(args.code_0)[0] # args.code_0 : false
    nt_id = tokenizer.encode(args.code_1)[0] # args.code_1 : true
    ## pt_id : 9562, nt_id : 7942
    ...
    for epoch_ in train_iterator:
        ...
        for step, batch in enumerate(epoch_iterator):
            ...
            torch.cuda.empty_cache()
            #batch is list of length 4
            #batch[0] (input ids) shape : tensor([batch size *n_gpu, 192])
            #batch[1] (attention mask) shape : tensor([2*n_gpu, 192]), batch[1][n] = [1, 1, 1, ..., 1, 0, 0, ..., 0]
            #batch[2] (token type ids) shape : tensor([2*n_gpu, 192]), batch[2][n] = [0, 0, 0, ..., 0]
            if args.sst5:
            ...
            if args.model_type == 'gpt2':
                ...
                # seq_a : tensor([[9562],[9562],[9562],[9562],[9562],[9562]])
                # seq_a : tensor([[7942],[7942],[7942],[7942],[7942],[7942]])

                if args.threeway:
                    seq_c = (torch.ones(batch_0.shape[0])*nl_id).type_as(batch_0).view(-1,1)

                seq_a = torch.cat((seq_a, batch_0), dim=1)[:,:-1]   # shape (6, 1+192)
                ...
                if args.threeway:
                    ...
                    # print('seq_batched.shape', seq_batched.shape)  #(12, 192)
            ...
            #loss mask : shape (6, 191) with each attentions (1, 1, ..., 0, 0)
            #loss_lengths : shape (6, 1)  ex. [42, 47, 58, 45, 43, 51]
            ...
            if args.do_adversarial == True and (step + 1) % 10 == 0 and step < 10000:
                model_for_gen = model.module 
                # model_for_gen = model_for_gen.to(args.device)
                ...
                gen_file = open('generated_sentence.txt', 'a')
                for i in range(10):  ##if generate more...it will take training so much time :(
                ...
                target_dataset = create_dataset('./generated_sentences/generated_sentence.txt', tokenizer, 0)
                optimizer_dis = optim.Adagrad(discriminator.parameters())
                train_discriminator(discriminator, optimizer_dis, source_dataset, target_dataset, d_steps=50, epochs=3)
            ...
            G_loss = loss_fn(class_logits, class_labels)*args.disc_weight + args.gen_weight*gen_loss  ##disc_weight:0.2, gen_weight : 0.8


            if np.isnan(G_loss.detach().cpu().numpy()):
                import pdb; pdb.set_trace()

            if args.n_gpu > 1:
                G_loss = G_loss.mean()  # mean() to average on multi-gpu parallel training
            if args.gradient_accumulation_steps > 1:
                G_loss = G_loss / args.gradient_accumulation_steps

            if args.fp16:
                with amp.scale_loss(G_loss, optimizer_gedi) as scaled_loss:
            ...
            if (step + 1) % args.gradient_accumulation_steps == 0:
                if args.fp16:
                    torch.nn.utils.clip_grad_norm_(amp.master_params(optimizer_gedi), args.max_grad_norm)
                else:
                ...
                if args.local_rank in [-1, 0] and args.save_steps > 0 and global_step % args.save_steps == 0:
                    ...
                    torch.save(optimizer_gedi.state_dict(), os.path.join(output_dir, "optimizer_gedi.pt"))
...
def load_and_cache_examples(args, task, tokenizer, evaluate=False):
    ...
    if os.path.exists(cached_features_file) and not args.overwrite_cache:
        ...
        # processor : <transformers.data.processors.glue.Sst2Processor object at 0x7f7233e74208>
        ...
        ## AG news topics are business, sports, science, world
        # examples : InputExample(guid='train-2399959', \
        # text_a='businessThere are no gates or fences, and \
            # most days anyone can drive straight to the warden #39;s home at the Federal Correctional Institute here.' \
        # text_b=None \
        # label='1')
...
#     examples = preprocess_text_to_examples(source_file)
#     # print(examples[:10])
...
def main():
    ...
    parser.add_argument("--mode", default="topic", type=str, help="attributes")
    ...
    parser.add_argument("--prompt", type=str, default="", help="Initially given text")
    parser.add_argument("--control_code", type=str, default="politics", help="control code")
...
#     args = parser.parse_args()
    ...
    ################
    ###GLUE tasks : cola, mnli, nnli-mm, mrpc, sst-2, sts-b, qqb, qnli, rte, wnli
    ################
    ...
    args.model_type = args.model_type.lower() #gpt2-medium
    ...
    # discrim_config_class, discrim_class, discrim_tokenizer_class = MODEL_CLASSES[args.discriminator]
    # discrim_config = config_class.from_pretrained(
    # 	args.discrim_name_or_path,
    #     num_labels=num_labels,
    #     finetuning_task=args.task_name,
    #     cache_dir=args.cache_dir if args.cache_dir else None,
    # )
    ...
    source_file = '/home/chaewon/GeDi/generated_sentences/gpt2_gen50000.txt'
    source_dataset = create_dataset(source_file, tokenizer, label=1)

    discriminator = Discriminator(embedding_dim=64, hidden_dim=64, vocab_size=50256, max_seq_len=50, gpu=True)
